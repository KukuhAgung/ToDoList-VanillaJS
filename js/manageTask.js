document.addEventListener("DOMContentLoaded", () => {
  ...
  function setUsername() {
    const usernameElement = document.querySelector(".profile-user");
  ...
  function displayAllTasks(tasks = existingTasks) {
    if (tasks.length === 0) {
      ...
      tasks.forEach((task) => {
          ...
          <div class="task-card flex flex-col gap-5">
            <div class="flex gap-3 items-center">
                ...
                <div class="flex flex-col">
                    <p class="font-bold text-lg leading-[27px] text-header">${capitalizeFirstChar(
                      task.taskName
                    )}</p> 
                    <p class="text-sm leading-[21px] text-paragrap">Created at ${userFriendlyDate}</p> 
                </div>
            </div>
            <div class="flex gap-4 font-semibold text-sm leading-[21px]">
                <div class="flex gap-1 items-center">
                    ...
                    <p class="text-paragrap">${task.taskPriority}</p> 
                </div>
                ${
                  task.isCompleted === false
                    ? `<div class="flex gap-1 items-center text-paragrap">
                          <div class="flex shrink-0 w-5 h-5">
                              <svg width="20" height="21" viewBox="0 0 20 21" fill="none"
                                  xmlns="http://www.w3.org/2000/svg">
                                  <path d="M4.29163 2.16663V18.8333" stroke="#7E7997" stroke-width="2"
                                  ...
                                  <path
                                      ...
                                      stroke="#7E7997" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round"
                                      stroke-linejoin="round" />
                              </svg>
                          </div>
                          <p>In Progress</p>
                      </div>`
                    : `<div class="flex gap-1 items-center text-completed">
                      <div class="flex shrink-0 w-5 h-5">
                        <svg
                          width="20"
                          height="21"
        ...
        class=" border-delete text-delete my-auto font-semibold text-taskia-red border border-taskia-red p-[12px_20px] h-12 rounded-full">Delete</a>
            ${
              task.isCompleted === false
                ? `<a href="#" id="completeTask-${task.id}"
                class=" border-none flex gap-[10px] justify-center items-center text-white p-[12px_20px] h-12 font-semibold bg-gradient-to-b from-[#977FFF] to-[#6F4FFF] rounded-full w-full border border-taskia-background-grey">Complete</a>`
                : `<a href="#" id="completeTask-${task.id}"
              class="hidden"></a>`
            }
        </div>
